...
def chat(request):
    ...
    # -------- OPEN APPLICATIONS --------
    if any(word in tokens for word in ["open", "launch", "start"]):
        app_name = None
        for ent in doc.ents:
        ...
        if not app_name:
            for i, token in enumerate(tokens):
                if token in ["open", "launch", "start"] and i + 1 < len(tokens):
                    app_name = tokens[i + 1].lower()
                    break
    ...
    # -------- TASK MANAGEMENT --------
        ...
        try:
            app_name = " ".join(tokens[tokens.index("kill") + 1:])
            if platform.system() == 'Windows':
